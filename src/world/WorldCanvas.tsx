...
export default function WorldCanvas() {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#87CEEB]">
        ...
        <Suspense fallback={null}>
          ...
          <EffectComposer>
            <Vignette 
              eskil={false} 
              offset={0.3} 
              darkness={activeWorld?.style.vignetteIntensity ?? 0.5} 
            />
            {activeWorld?.style.grain && (
              <Noise opacity={activeWorld?.style.grainIntensity ?? 0.02} />
            )}
            <Bloom intensity={0.5} luminanceThreshold={1} />
            {activeWorld?.style.halftone && <DotScreen angle={Math.PI * 0.25} scale={1.0} />}
          </EffectComposer>
        </Suspense>
      </Canvas>
    </div>
